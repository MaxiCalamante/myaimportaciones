...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string; page?: string; sort?: string }> }) {
  ...
  function pageUrl(n: number) { const query = new URLSearchParams({ q, category: category?.slug ?? "", sort: params.sort ?? "", page: String(n) }); return `/catalogo?${query}`; }
  return <div className="mx-auto max-w-7xl px-4 py-10">
    ...
    <form className="my-6 flex flex-wrap gap-3" action="/catalogo">
      ...
      <select aria-label="Categoría" name="category" defaultValue={category?.slug ?? ""} className="max-w-full rounded-xl border p-3"><option value="">Todas las categorías</option>{categories.filter(c => !c.wholesaleOnly).map(c => <option key={c.id} value={c.slug}>{c.name}</option>)}</select>
