  ...
  const itemsXml = products
    .map((p) => {
      ...
      const cleanDesc = p.description
        ...
        : escapeXml(`${p.title} importado directamente por MYA Importaciones.`);
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    <title>MYA Importaciones - Catálogo de Productos para Meta Ads</title>
    <link>${escapeXml(baseUrl)}</link>
    <description>Catálogo sincronizado en tiempo real para campañas de Meta Ads (Facebook e Instagram Shopping) de MYA Importaciones.</description>
${itemsXml}
  </channel>
</rss>`;
