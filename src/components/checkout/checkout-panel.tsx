  ...
  return <section className="mx-auto max-w-6xl px-4 py-10"><h1 className="text-3xl font-bold">Finalizar compra</h1><p className="mt-2 text-slate-600">Revisá entrega y total antes de confirmar. La compra es minorista.</p>
    ...
    <form className="mt-8 grid items-start gap-8 lg:grid-cols-[1fr_380px]" onSubmit={e => {
      ...
      <div className="space-y-6 rounded-2xl border bg-white p-5"><h2 className="text-xl font-bold">Entrega y contacto</h2>
        ...
        {shippingCalculation.isValid && <label className="block">Forma de entrega<select className={field} value={selectedShippingOption?.id ?? ""} onChange={e => setSelectedShippingOptionId(e.target.value)}>{shippingCalculation.options.map(o => <option key={o.id} value={o.id}>{o.name} — {formatCurrency(o.price)}</option>)}</select><span className="mt-2 block text-xs text-slate-600">Tarifas de la tienda para paquetes hasta 2 kg con peso registrado. Bultos grandes requieren cotización. Plazos de transporte estimados desde el despacho.</span></label>}
